...
#define NUMBER_OF_ANALYSIS_VALUES 7
#define AN_MASS                   0
#define AN_E                      1
#define AN_PERI                   2
 ...
 *********************************************************************** */
{
    ...
    double R_gap = 2.5*g_inputParam[A_BIN];
    double delta_R = 0.1*R_gap;
    double f_gap = 1.0/(1.0 + exp(-(x1-R_gap)/delta_R));

    v[RHO] = f_gap*g_inputParam[SIGMA_REF]*pow(x1, -g_inputParam[ALPHA_SIGMA]);
    v[VX1] = 0.0;
 ...
 *********************************************************************** */
{
    ...
    if (prank == 0)
    {
        ...
        if (g_stepNumber == 0)
        {
            ...
            fprintf(fp, "#%-11s  %-12s  %-12s  %-12s  %-12s  %-12s  %-12s  %-12s\n", 
