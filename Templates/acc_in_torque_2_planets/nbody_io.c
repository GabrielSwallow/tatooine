...
int nbodyReadPlanetFile(const char *filename)
{
    ...
    while (fgets(line_buffer, BUFFER_LENGTH, f) != NULL)
    {
        ...
        if (sscanf(line_buffer, "%lf %lf %lf %lf %lf %lf %lf %d", 
                   &(g_nb.m[i]), &a, &e, &inc, &Omega, &omega, &f, &(g_nb.feelsDisk[i])) != 8)
        {
            fprintf(stderr, "Error in read_planet_file: Not enough values in line %d\n", i);
            goto read_planet_file_error;
        }
...
void nbodyWriteOrbitalElements()
{
    if (prank == 0)
    {
        ...
        if (g_time > tpos)
        {
            ...
            for (int l = 1; l < NB_N; l++)
            {
                fprintf(fp, "%-3d  %- 12.6e  %- 12.6e  %- 12.6e  %- 12.6e  %- 12.6e  %- 12.6e  %- 12.6e  %- 12.6e  %- 12.6e  %- 12.6e  %- 12.6e\n",
                        l,
                        g_time,
                        g_nb.a[l], 
                        g_nb.e[l], 
...
void nbodyWriteRestartFile(Output *output)
{
    ...
    if (prank == 0)
    {
        ...
        else
        {
            ...
            for (int nv = 0; nv < output->nfile; nv++) 
            {
                for (int l = 0; l < NB_N; l++)
                {
